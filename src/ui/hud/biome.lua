...
function HUD.drawBiomeInfoPanel()
    if not HUD.player or not HUD.BiomeSystem then return end
    
    local currentTime = love.timer.getTime()
    local scanInterval = HUD.hudState.performance.reducedUpdateMode and 2.0 or 1.0
    
    if not HUD.lastBiomeScan or currentTime - HUD.lastBiomeScan > scanInterval then
        HUD.nearbyBiomes = HUD.BiomeSystem.findNearbyBiomes(HUD.player.x, HUD.player.y, 10000)
        HUD.lastBiomeScan = currentTime
        
        if HUD.hudState.performance.enableCaching then
            HUD.hudState.renderCache.cachedBiomeInfo = HUD.nearbyBiomes
        end
    end
    ...
    local biomeInfo = HUD.biomeCache.currentBiomeInfo
    
    if HUD.biomeCache.debugInfo and not biomeInfo then
        ...
        love.graphics.print("Player exists: " .. tostring(HUD.biomeCache.debugInfo.playerExists), x + 15, debugY)
        debugY = debugY + 12
        love.graphics.print("Player coords: " .. tostring(HUD.biomeCache.debugInfo.playerHasCoords), x + 15, debugY)
        debugY = debugY + 12
        love.graphics.print("BiomeSystem: " .. tostring(HUD.biomeCache.debugInfo.biomeSystemExists), x + 15, debugY)
        debugY = debugY + 12
        love.graphics.print("getPlayerBiomeInfo: " .. tostring(HUD.biomeCache.debugInfo.getPlayerBiomeInfoExists), x + 15, debugY)
        debugY = debugY + 12
        
        if HUD.biomeCache.debugInfo.playerCoords then
            love.graphics.print(string.format("Coords: (%.1f, %.1f)", 
                HUD.biomeCache.debugInfo.playerCoords.x, HUD.biomeCache.debugInfo.playerCoords.y), x + 15, debugY)
            debugY = debugY + 12
        end
        
        if HUD.biomeCache.lastError then
            love.graphics.setColor(1, 0.5, 0.5, 1)
            love.graphics.print("Error: " .. HUD.biomeCache.lastError, x + 15, debugY)
        end
    end
    ...
    if biomeInfo then
        ...
        for i = 1, itemsToShow do
            ...
            local displayText = HUD.hudState.stringPool[stringKey]
            if not displayText then
                displayText = string.format("%s (%d m)", biome.name, distance)
                if #HUD.hudState.stringPool < HUD.hudState.performance.maxStringPoolSize then
                    HUD.hudState.stringPool[stringKey] = displayText
                end
            end
