...
function HUD.drawSeedInput()
    ...
    for i, preset in ipairs(HUD.presetSeeds) do
        local color = (i == HUD.currentPresetIndex) and {1, 1, 0.3, 1} or {0.8, 0.8, 0.8, 1}
        local prefix = (i == HUD.currentPresetIndex) and "> " or "  "
    ...
    love.graphics.setColor(1, 1, 1, 1)
    local displayText = HUD.hudState.seedInputText:upper()
    love.graphics.print(displayText, x + 25, y + 305)
    
    if math.floor(love.timer.getTime() * 2) % 2 == 0 then
        local textWidth = HUD.hudState.font:getWidth(displayText)
        love.graphics.line(x + 25 + textWidth, y + 305, x + 25 + textWidth, y + 320)
    end
    
    local isValid = HUD.SeedSystem.validate(displayText)
    ...
    love.graphics.setColor(validColor[1], validColor[2], validColor[3], validColor[4])
    ...
    love.graphics.setColor(0.7, 0.7, 0.7, 1)
    love.graphics.print("Letters A-Z and digits 0-9 only \226\128\162 Enter to confirm \226\128\162 Escape to cancel", x + 20, y + 355)
...
function HUD.drawCurrentSeedInfo()
    if not HUD.gameState then return end
    
    local currentSeed = HUD.gameState.currentSeed or "UNKNOWN00"
    ...
    local isValid = HUD.SeedSystem.validate(currentSeed)
...
function HUD.handleSeedInput(key)
    if key == "escape" then
        HUD.hudState.showSeedInput = false
        HUD.hudState.seedInputText = ""
    elseif key == "return" or key == "enter" then
        if HUD.hudState.seedInputText ~= "" then
            local normalizedSeed = HUD.SeedSystem.normalize(HUD.hudState.seedInputText)
            if HUD.SeedSystem.validate(normalizedSeed) then
                HUD.hudState.showSeedInput = false
                HUD.hudState.seedInputText = ""
            ...
            local selectedPreset = HUD.presetSeeds[HUD.currentPresetIndex]
            local selectedSeed = selectedPreset.seed
            if selectedPreset.name == "Random" then
                selectedSeed = HUD.SeedSystem.generate()
    ...
    elseif key == "backspace" then
        HUD.hudState.seedInputText = string.sub(HUD.hudState.seedInputText, 1, -2)
    elseif key == "up" then
        HUD.currentPresetIndex = math.max(1, HUD.currentPresetIndex - 1)
    elseif key == "down" then
        HUD.currentPresetIndex = math.min(#HUD.presetSeeds, HUD.currentPresetIndex + 1)
    end
    return nil, nil
end
...
function HUD.textinput(text)
    if HUD.hudState.showSeedInput then
        ...
        if upperText:match("[A-Z0-9]") and #HUD.hudState.seedInputText < 10 then
            HUD.hudState.seedInputText = HUD.hudState.seedInputText .. upperText
        end
    end
end

function HUD.showSeedInput()
    HUD.hudState.showSeedInput = true
    HUD.hudState.seedInputText = ""
    HUD.presetSeeds[1].seed = HUD.SeedSystem.generate()
end
