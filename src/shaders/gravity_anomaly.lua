                ...
                if activeCount < 2 then
                    ...
                    if rng:random() < GravityAnomaly.config.lens.spawnChance * 0.25 then  -- Reducir probabilidad para evitar spam
                        local lens = {
                            ...
                            y = rng:random() * MapConfig.chunk.size * MapConfig.chunk.tileSize,
                            size = rng:random() * (GravityAnomaly.config.lens.maxSize - GravityAnomaly.config.lens.minSize) + GravityAnomaly.config.lens.minSize,
                            startTime = currentTime + rng:random() * 3.0,
                            duration = GravityAnomaly.config.lens.animationDuration,
                            rotation = rng:random() * math.pi * 2,
                            rotationSpeed = (rng:random() - 0.5) * 0.5,
                            chunkX = chunkX,
                            chunkY = chunkY
                        }
                        table.insert(chunk.gravityAnomalies, lens)
                ...
                for _, anomaly in ipairs(chunk.gravityAnomalies) do
                    ...
                    if drawThis then
                        if anomaly.type == "wave" then
                            GravityAnomaly.drawWaveAnomaly(anomaly, screenX, screenY, pxSize)
                        elseif anomaly.type == "lens" then
                            GravityAnomaly.drawLensAnomaly(anomaly, screenX, screenY, pxSize)
                        end
                        rendered = rendered + 1
                    end
                end
...
function GravityAnomaly.drawContinuousAnomalies(camera)
    ...
    -- Debug: contar anomalías activas
    ...
    if not tracker.lastDebugTime then tracker.lastDebugTime = 0 end
    if currentTime - tracker.lastDebugTime >= 3.0 then
        tracker.lastDebugTime = currentTime
        print(string.format("[GravityAnomaly DEBUG] Anomalías continuas: %d total, %d activas, en bioma: %s", 
            #tracker.continuousAnomalies, active, tostring(tracker.isInGravityBiome)))
    end
    
    for i, anomaly in ipairs(tracker.continuousAnomalies) do
        local elapsed = currentTime - anomaly.startTime
        
        -- Solo renderizar anomalías que están en su tiempo de vida
        if elapsed >= 0 and elapsed <= anomaly.duration then
            -- Convertir a pantalla una sola vez (igual que asteroides)
            ...
            if drawThis then
                if anomaly.type == "wave" then
                    GravityAnomaly.drawWaveAnomaly(anomaly, screenX, screenY, pxSize, elapsed)
                elseif anomaly.type == "lens" then
                    GravityAnomaly.drawLensAnomaly(anomaly, screenX, screenY, pxSize, elapsed)
                end
                rendered = rendered + 1
                
                -- Debug: log de renderizado ocasional
                if i == 1 and currentTime - (tracker.lastRenderDebug or 0) >= 2.0 then
                    tracker.lastRenderDebug = currentTime
                    print(string.format("[GravityAnomaly DEBUG] Renderizando anomalía %s #%d: elapsed=%.2f/%.2f, pos=(%.1f,%.1f)", 
                        anomaly.type, i, elapsed, anomaly.duration, anomaly.x, anomaly.y))
                end
            end
        end
    end
...
function GravityAnomaly.updateContinuousAnomalies(camera)
    ...
    -- Debug: verificar estado del bioma en updateContinuousAnomalies
    if not tracker.lastBiomeDebug then tracker.lastBiomeDebug = 0 end
    if currentTime - tracker.lastBiomeDebug >= 2.0 then
        tracker.lastBiomeDebug = currentTime
        print(string.format("[GravityAnomaly DEBUG] updateContinuousAnomalies - isInGravityBiome: %s", tostring(tracker.isInGravityBiome)))
    end
    
    -- Si el jugador está en el bioma gravity_anomaly, generar anomalías continuas
    if tracker.isInGravityBiome then
        ...
        -- Inicializar nextSpawnTime si es la primera vez en el bioma
        if tracker.nextSpawnTime == 0 then
            -- Spawn más rápido al entrar al bioma (0.3-0.8 segundos)
            local quickSpawn = 0.3 + math.random() * 0.5
            tracker.nextSpawnTime = currentTime + quickSpawn
        end
        
        -- Generar nueva anomalía si es tiempo y no hemos alcanzado el límite
        if currentTime >= tracker.nextSpawnTime and activeCount < tracker.maxContinuousAnomalies then
            local jitter = (math.random() - 0.5) * tracker.spawnInterval * 0.4  -- ±40% de variación
            tracker.nextSpawnTime = currentTime + tracker.spawnInterval + jitter
            
            -- Elegir una posición aleatoria dentro del rectángulo visible (expandido) y validar el bioma
            local cam = camera or (_G and _G.camera)
            local left, top, right, bottom
            if cam and type(cam.screenToWorld) == "function" then
                local w, h = love.graphics.getWidth(), love.graphics.getHeight()
                local marginPx = 1200 -- margen para abarcar más allá de la pantalla
                local wl, wt = cam:screenToWorld(-marginPx, -marginPx)
                local wr, wb = cam:screenToWorld(w + marginPx, h + marginPx)
                left, right = math.min(wl, wr), math.max(wl, wr)
                top, bottom = math.min(wt, wb), math.max(wt, wb)
            else
                -- Fallback: área amplia alrededor de la última posición conocida del jugador
                local centerX = tracker.lastPlayerX or 0
                local centerY = tracker.lastPlayerY or 0
                local spread = 3000
                left, right = centerX - spread, centerX + spread
                top, bottom = centerY - spread, centerY + spread
            end
            ...
            -- Intentar algunas veces encontrar un punto dentro del bioma correcto
            for _ = 1, 6 do
            ...
            -- Si no se encontró, usar un punto cercano al centro del área como degradado
            if not x or not y then
                x = (left + right) * 0.5 + (math.random() - 0.5) * 1000
                y = (top + bottom) * 0.5 + (math.random() - 0.5) * 1000
            end
            
            -- Elegir tipo aleatoriamente y crear la anomalía (tamaños base heredados del sistema de dibujo fijo)
